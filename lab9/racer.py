...
player_img = pygame.image.load('lab8/images/Player.png')
enemy_img = pygame.image.load('lab8/images/Enemy.png')
coin_img = pygame.image.load('lab8/images/coin.png')

background_music = pygame.mixer.music.load('lab8/sound/background.wav')
crash_sound = pygame.mixer.Sound('lab8/sound/crash.wav')
...
PLAYER_SPEED = 5
ENEMY_SPEED = 5  # Starting enemy speed is low
score = 0
coin_score = 0
...
# Coin class to generate coins that fall down, with random weight and score value
class Coin(pygame.sprite.Sprite):
    def __init__(self, enemy):
        ...
        # Setting a fixed size for all coins
        self.image = pygame.transform.scale(self.image, (40, 40))  # Adjust size as needed
        
        # Random weight for the coin (which will affect its score)
        self.weight = random.randint(1, 3)  # Coins with higher weight will give more points
        self.score_value = self.weight * 10  # Assign points based on the weight (e.g., 10, 20, or 30 points)
...
while running:
    ...
    for collected_coin in collected_coins:
        coin_score += collected_coin.score_value  # Increment the coin score by the coin's weight-based value
        ...
        # Increase enemy speed after collecting 2 coins
        if coin_score % 2 == 0:  # Every 2 coins
            ENEMY_SPEED += 1  # Increase the enemy speed gradually
